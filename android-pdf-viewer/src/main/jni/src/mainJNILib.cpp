...
class DocumentFile {
    private:
    int fileFd;
    FPDF_FORMFILLINFO formFillInfo;  // Must persist for lifetime of formHandle
...
JNI_FUNC(jlong, PdfiumCore, nativeOpenDocument)(JNI_ARGS, jint fd, jstring password){
    ...
    docFile->initFormFillEnvironment();
...
JNI_FUNC(void, PdfiumCore, nativeRenderPageBitmap)(JNI_ARGS, jlong docPtr, jlong pagePtr, jobject bitmap,
    ...
    // Render form fields (checkboxes, text fields, etc.)
    if(doc != NULL && doc->formHandle != NULL && renderAnnot) {
